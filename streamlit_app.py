...
with tab_gate:
    ...
    with box:
        p1_threshold = st.slider(
            ...
            help="릴리즈마다 리스크 허용치가 다를 수 있어 직접 조정합니다.\n\n"
    ...
    with box:
        with st.expander("판단 기준 보기"):
            ...
            score_tbl = pd.DataFrame({
                ...
                "중": ["재시도로 복구 가능한 주요기능 실패", "특정 조건·세그먼트(멀티펫·해외·네트워크)",
                      "우회는 되나 사용자 혼자 못 벗어남"],
...
# ────────────────────────────────────────────────────────────
with tab_repro:
    box = section("재현성 검증", key="repro",
                   caption="건수·P0 비율은 기획서 리스크 구조에 따라 다르지만, 판정 규칙은 4건 모두 동일합니다.")
    with box:
        rows = []
